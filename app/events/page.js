...
export default function EventsPage() {
  ...
  const handleBook = async (eventId) => {
    ...
    if (res.ok) {
      // Optimistically update seats and show success message
      setEvents((prev) => prev.map((e) => (e.id === eventId ? { ...e, seats: Math.max(0, (e.seats ?? 0) - qty) } : e)));
      setMessages((m) => ({ ...m, [eventId]: 'Booked successfully!' }));
      // Auto-clear success after a short delay
      setTimeout(() => {
        setMessages((m) => {
          const { [eventId]: _omit, ...rest } = m;
          return rest;
        });
      }, 2500);
    } else {
  ...
  return (
    <div className="bg-slate-50 min-h-screen px-6 py-8 sm:px-8 sm:py-10 lg:px-12 lg:py-12">
      <button
        type="button"
        aria-label="Go back"
        onClick={() => router.back()}
        ...
        {(Array.isArray(events) ? events : []).map((event) => (
          <div key={event.id} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm hover:shadow-md transition">
            ...
            <div className="mt-2 text-sm text-slate-600">
              ...
              {messages[event.id] && (
                <p className="mt-2 text-green-600 text-sm">{messages[event.id]}</p>
              )}
            </div>
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                  ...
                  <div className="inline-flex items-center rounded-lg border border-slate-300 bg-slate-50 overflow-hidden shadow-sm">
                    <button
                      ...
                      className={`h-9 px-2.5 text-slate-700 hover:bg-slate-100 focus:outline-none focus:bg-slate-100 ${
                        (quantities[event.id] ?? 1) <= 1 ? 'opacity-40 cursor-not-allowed' : ''
                      }`}
                    >
                      −
                    </button>
                    <div className="h-9 w-10 flex items-center justify-center bg-white text-slate-900 font-medium border-x border-slate-300">
